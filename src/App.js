...
function App() {
  const [lists, setLists] = React.useState(
    DB.lists.map(item => {
      item.color = DB.colors.find(color =>
        color.id === item.colorId).name;
      return item;
    })
  );
      ...
      <div className="todo__sidebar">
        ...
        {lists && (
          <List
            ...
            isRemovable
          />)}
        <AddList colors={DB.colors} onAdd={(obj) => onAddList(obj)} isRemovable={false} />
      </div>
      <div className="todo__tasks">
        {lists && (<Tasks tasks={DB.tasks} lists={lists[1]}/>)}
      </div>
    </div>
  );
}
